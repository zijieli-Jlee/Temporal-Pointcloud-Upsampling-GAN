"""Helper functions for reading and writing simulation sim_data"""
import os
import re
...
def numpy_from_bgeo(path, read_dns=False):
    import partio
    p = partio.read(path)
    pos = p.attributeInfo('position')
    vel = p.attributeInfo('velocity')
    dns = p.attributeInfo('density')
    ...
    if read_dns:
        dns_arr = np.empty((n, pos.count))
        if not dns is None:
            for i in range(n):
                dns_arr[i] = p.get(dns, i)
    ...
    if not ida is None:
        ...
        if not dns is None:
            result.append(dns_arr[s])
    else:
        result = [pos_arr, vel_arr]         # , prs_arr]
